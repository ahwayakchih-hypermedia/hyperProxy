...
var certutil = require('./CertUtil.js');
...
function Proxy (options) {
	...
	options.installRootCert = options.hasOwnProperty('installRootCert') ? options.installRootCert : true;
	...
	this.initializeRootCertificate = function (callback) {
		...
		var onRead = self.whenAllDone.bind(self, {id: 'root-certificate-from-files', todo: 2, done: 0}, function () {
			...
			else if (options.installRootCert) {
				certutil.add('Root', options.certPath, function (err) {
					if (err) {
						console.warn('Could not install root certificate', err);
					}
					else {
						console.log('Installed root certificate. Make sure it is uninstalled after you finish using proxy because it makes you a bit less safe.');
					}
				});
			}
	...
	this.stop = function (callback) {
		...
		var todo = 1 + (self.httpsServer ? 1 : 0) + (options.certPath && options.installRootCert ? 1 : 0);
		var onClosed = self.whenAllDone.bind(self, {id: 'closing', todo: todo, done: 0}, function () {
		...
		if (options.certPath && options.installRootCert) {
			certutil.remove('Root', options.certPath, function (err) {
				if (err) {
					console.warn('Could not uninstall root certificate ' + options.certPath);
				}
				else {
					console.log('Uninstalled root certificate. You are a bit safer now again.');
				}
				onClosed();
			});
		}
