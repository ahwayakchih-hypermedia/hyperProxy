...
var ProxyAgent = require('./ProxyAgent.js')();
...
function Proxy (options) {
	...
	this.initializeRootCertificate = function (callback) {
		...
		var onRead = self.whenAllDone.bind(self, {id: 'root-certificate-from-files', todo: 2, done: 0}, function () {
			...
			pem.readCertificateInfo(options.cert, function (err, data) {
				if (err) {
					console.error('Could not read root certificate info from ' + options.certPath);
					console.error(err);
					options.cert = null;
				}
				else {
					var now = Date.now();
					if (now < data.validity.start || now >= data.validity.end) {
						_createRootCertificate();
						return;
					}
				}
